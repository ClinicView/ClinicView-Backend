...
describe('Integridad clínica real y aislada (e2e)', () => {
  ...
  it('busca en todas las páginas, respeta fechas clínicas y limita fuentes según permisos', async () => {
    await prisma.clinicalRecord.createMany({ data: Array.from({ length: 61 }, (_, index) => ({ patientId, recordType: 'CONSULTATION', attendedAt: new Date(index === 60 ? '2021-01-01T05:00:00Z' : '2024-01-01T05:00:00Z'), summary: `Zafiro analítico ${index}`, details: VALID_RECORD_DETAILS.CONSULTATION as Prisma.InputJsonValue, doctorName: 'Profesional de prueba', createdBy: fixture.clinician.id })) });
    type Page = { data: Array<{ id: string; kind: string; preview: string; clinicalFrom: string | null }>; total: number };
    const search = (query: string, token = readerToken) => jsonRequest<Page>(baseUrl, `/api/patients/${patientId}/clinical-history/search?${query}`, { headers: jsonHeaders(token) });
    const first = await search('q=zafiro%20analitico');
    expect(first.response.status).toBe(200);
    expect(first.body.total).toBe(61);
    expect(first.body.data).toHaveLength(20);
    const pages = await Promise.all([2, 3, 4].map((page) => search(`q=zafiro%20analitico&page=${page}`)));
    expect(new Set([...first.body.data, ...pages.flatMap((page) => page.body.data)].map((row) => row.id)).size).toBe(61);
    ...
    const original = await uploadMultipart<DocumentResponse>(`/api/patients/${patientId}/documents`, clinicianToken, VALID_PDF, 'rango-busqueda-e2e.pdf', 'application/pdf');
    await prisma.medicalDocument.update({ where: { id: original.body.id }, data: { status: 'VALIDATED', correctedText: 'Contenido antiguo zafiro documental', clinicalMetadata: { clinicalDate: '2020-01-01', clinicalEndDate: '2022-12-31' } } });
    expect((await search('q=zafiro%20documental&from=2021-01-01&to=2021-12-31')).body.total).toBe(1);
    await prisma.medicalDocument.update({ where: { id: original.body.id }, data: { clinicalMetadata: {} } });
    expect((await search('q=zafiro%20documental&from=2021-01-01')).body.total).toBe(0);
    await prisma.medicalDocument.update({ where: { id: original.body.id }, data: { status: 'PROCESSED' } });
    ...
    const permissions = await prisma.permission.findMany({ where: { key: { in: ['patients.read', 'records.read'] } } });
    const role = await prisma.role.create({ data: { key: 'E2E_RECORDS_ONLY_SEARCH', name: 'Lectura solo de atenciones', rolePermissions: { create: permissions.map(({ id }) => ({ permissionId: id })) } } });
    const user = await prisma.user.create({ data: { email: 'search-records@clinicview.invalid', username: 'e2e_search_records', firstName: 'Lectura', lastName: 'Sintética', fullName: 'Lectura Sintética', passwordHash: owner.passwordHash, userRoles: { create: { roleId: role.id } } } });
    const token = await login({ id: user.id, email: user.email, username: user.username, password: fixture.reader.password });
    ...
    const overview = await jsonRequest<{ recordVersions: number; activeRecords: number; documents: number | null; latestClinicalDate: string | null }>(baseUrl, `/api/patients/${patientId}/clinical-history/overview`, { headers: jsonHeaders(token) });
    expect(overview.response.status).toBe(200);
    expect(overview.body.recordVersions).toBe(await prisma.clinicalRecord.count({ where: { patientId } }));
  ...
  it('exporta selecciones explícitas conservando originales citados y trazabilidad de episodios', async () => {
    type Export = ClinicalHistoryExport & { scope: { kind: string; description: string; includesSourceDocumentsOutsidePeriod: boolean }; episodes: Array<{ id: string; events: Array<{ action: string; actorName: string }> }> };
    const request = (query: string) => jsonRequest<Export>(baseUrl, `/api/patients/${patientId}/clinical-history/export?${query}`, { headers: jsonHeaders(readerToken) });
    const complete = await request('');
    expect(complete.response.status).toBe(200);
    expect(complete.body.scope.kind).toBe('COMPLETE');
    expect(complete.body.records.length).toBe(await prisma.clinicalRecord.count({ where: { patientId } }));
    const source = await prisma.clinicalRecordSource.findFirstOrThrow({ where: { record: { patientId, status: 'ACTIVE' } } });
    await prisma.medicalDocument.update({ where: { id: source.documentId }, data: { clinicalMetadata: { clinicalDate: '2020-01-01' }, status: 'PENDING' } });
    const selected = await request('from=2023-09-27&to=2023-09-27&versions=CURRENT');
    expect(selected.response.status).toBe(200);
    expect(selected.body.scope).toMatchObject({ kind: 'FILTERED', includesSourceDocumentsOutsidePeriod: true });
    expect(selected.body.records.every((record) => record.status === 'ACTIVE')).toBe(true);
    expect(selected.body.documents.find((document) => document.id === source.documentId)?.clinicalText).toBeNull();
    expect(selected.body.documents.some((document) => document.id === source.documentId)).toBe(true);
    ...
    expect(grouped.body.episodes[0].events.map((event) => event.action)).toEqual(expect.arrayContaining(['CREATE', 'CLOSE', 'REOPEN', 'CORRECT']));
    expect(grouped.body.episodes[0].events.every((event) => Boolean(event.actorName))).toBe(true);
    expect(grouped.body.records.length).toBe(await prisma.clinicalRecord.count({ where: { patientId, episodeId: episode.id } }));
  ...
  it('muestra pendientes personales vivos, paginados y sin confundirlos con notificaciones leídas', async () => {
    ...
    const tasks = (query: string, token = clinicianToken) => jsonRequest<Page>(baseUrl, `/api/clinical-work?${query}`, { headers: jsonHeaders(token) });
    ...
    const row = await prisma.clinicalRecord.findFirstOrThrow({ where: { patientId, summary: 'Zafiro analítico 60' } });
    const confirmed = await jsonRequest<RecordResponse>(baseUrl, `/api/patients/${patientId}/records/${row.id}/confirm`, { method: 'POST', headers: jsonHeaders(clinicianToken), body: JSON.stringify({ expectedVersion: row.version, attested: true }) });
    expect(confirmed.response.status).toBe(201);
    expect((await tasks('kind=CONFIRMATION')).body.total).toBe(pending.body.total - 1);
    const draft = await prisma.clinicalRecordDraft.upsert({ where: { patientId_actorId: { patientId, actorId: fixture.clinician.id } }, create: { patientId, actorId: fixture.clinician.id, expiresAt: new Date(Date.now() + 3600000) }, update: { expiresAt: new Date(Date.now() + 3600000) } });
    expect((await tasks('kind=DRAFT')).body.data.some((item) => item.resourceId === draft.id)).toBe(true);
    expect((await tasks('kind=DRAFT', peerToken)).body.data.some((item) => item.resourceId === draft.id)).toBe(false);
    await prisma.clinicalRecordDraft.update({ where: { id: draft.id }, data: { expiresAt: new Date(Date.now() - 10000) } });
    expect((await tasks('kind=DRAFT')).body.data.some((item) => item.resourceId === draft.id)).toBe(false);
    const assigned = await prisma.medicalDocument.findFirstOrThrow({ where: { patientId, originalName: 'rango-busqueda-e2e.pdf' } });
    await prisma.medicalDocument.update({ where: { id: assigned.id }, data: { status: 'PROCESSED', assignedReviewerId: fixture.clinician.id, assignedAt: new Date() } });
    expect((await tasks('kind=DOCUMENT')).body.data.some((item) => item.resourceId === assigned.id)).toBe(true);
    expect((await tasks('kind=DOCUMENT', peerToken)).body.data.some((item) => item.resourceId === assigned.id)).toBe(false);
    await prisma.medicalDocument.update({ where: { id: assigned.id }, data: { status: 'VALIDATED' } });
    expect((await tasks('kind=DOCUMENT')).body.data.some((item) => item.resourceId === assigned.id)).toBe(false);
  });
